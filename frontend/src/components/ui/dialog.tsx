...
>(({ className, ...props }, ref) => (
  <DialogPrimitive.Overlay
    ref={ref}
    className={cn(
      "fixed inset-0 z-50 backdrop-blur-sm bg-background/80 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0",
      className
    )}
    {...props}
  />
  ...
  <DialogPortal>
    ...
    <DialogPrimitive.Content
      ...
      onPointerDownOutside={(e) => {
        // Prevent dialog dismiss when clicking on the tour tooltip
        const tooltip = document.querySelector("[data-tour-tooltip]");
        if (tooltip && tooltip.contains(e.target as Node)) {
          e.preventDefault();
        }
        // Also call any passed-in handler
        onPointerDownOutside?.(e);
      }}
      {...props}
    >
