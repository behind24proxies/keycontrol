...
export default function KeyCasesPage() {
  ...
  const [presetFormData, setPresetFormData] = useState({ ...presetDefaultForm });
      ...
      const projectsWithGroups = await Promise.all(
        pRes.data.map(async (p: any) => {
          try {
            ...
            return { id: p.id, name: p.name, endpoint_groups: detail.data.endpoint_groups || [] };
          } catch {
  ...
  const handlePresetCreate = async (e: React.FormEvent) => {
    ...
    try {
      const payload = {
        ...
        rate_limit_id: presetFormData.rate_limit_id ? parseInt(presetFormData.rate_limit_id) : null,
        ip_allowlist_id: presetFormData.ip_allowlist_id ? parseInt(presetFormData.ip_allowlist_id) : null,
        ip_blocklist_id: presetFormData.ip_blocklist_id ? parseInt(presetFormData.ip_blocklist_id) : null,
      ...
      toast({ title: "Preset created", description: `"${res.data.name}" created and selected` });
    } catch (error: any) {
      toast({
        variant: "destructive",
        title: "Error",
  ...
  return (
    <div className="p-8">
                        ...
                        <Tooltip>
                          ...
                          <TooltipContent>
                            <p className="max-w-xs">
                              ...
                              limits, and IP restrictions this API key will
                              have.
                            </p>
                          </TooltipContent>
                        </Tooltip>
      ...
      {/* Full Preset Creation Dialog */}
      <Dialog open={presetCreateOpen} onOpenChange={(o) => { setPresetCreateOpen(o); if (!o) setPresetFormData({ ...presetDefaultForm }); }}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>Create Preset</DialogTitle>
            <DialogDescription>
              Define a new access control preset with resources, rate limits, and IP restrictions.
            </DialogDescription>
          </DialogHeader>
          <form onSubmit={handlePresetCreate}>
            <div className="space-y-5 py-4">
              ...
              <div>
                ...
                <Select
                  ...
                  onValueChange={(v) => setPresetFormData({ ...presetFormData, rate_limit_id: v === "none" ? "" : v })}
                >
                  <SelectTrigger>
                  ...
                  <SelectContent>
                    ...
                    {rateLimits.map((rl) => (
                      <SelectItem key={rl.id} value={rl.id.toString()}>{rl.name}</SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              </div>
              ...
              <div>
                ...
                <Select
                  ...
                  onValueChange={(v) => setPresetFormData({ ...presetFormData, ip_allowlist_id: v === "none" ? "" : v })}
                >
                  <SelectTrigger>
                    <SelectValue placeholder="None" />
                  </SelectTrigger>
                  <SelectContent>
                    <SelectItem value="none">None</SelectItem>
                    {ipAllowlists.map((al) => (
                      <SelectItem key={al.id} value={al.id.toString()}>{al.name}</SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              </div>
              ...
              <div>
                ...
                <Select
                  ...
                  onValueChange={(v) => setPresetFormData({ ...presetFormData, ip_blocklist_id: v === "none" ? "" : v })}
                >
                  <SelectTrigger>
                    <SelectValue placeholder="None" />
                  </SelectTrigger>
                      ...
                      <span className="flex gap-2 text-xs text-muted-foreground">
                        {presetFormData.resource_ids.length > 0 ? (
                          <span className="px-2 py-0.5 bg-primary/10 text-primary rounded-full">
                            {presetFormData.resource_ids.length} resource{presetFormData.resource_ids.length !== 1 ? "s" : ""}
                          </span>
                        ) : (
                        ...
                        {presetFormData.endpoint_group_ids.length > 0 && (
                          <span className="px-2 py-0.5 bg-primary/10 text-primary rounded-full">
                            {presetFormData.endpoint_group_ids.length} group{presetFormData.endpoint_group_ids.length !== 1 ? "s" : ""}
                          </span>
                        )}
                      </span>
                    </Button>
            ...
            <DialogFooter>
              <Button type="button" variant="outline" onClick={() => setPresetCreateOpen(false)}>
                Cancel
              </Button>
              <Button type="submit" disabled={!presetFormData.name.trim() || presetCreateLoading}>
