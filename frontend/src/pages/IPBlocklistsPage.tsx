...
export default function IPBlocklistsPage() {
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      ...
      const res = await api.get(`/ip-blocklists/${id}/associated-keys`);
  ...
  const handleEdit = (blocklist: any) => {
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <TooltipProvider>
                      <Tooltip>
                        ...
                        <TooltipContent>
                          <p className="max-w-xs">
                            The content type of the response body when an IP is blocked. 
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <Card className="border-dashed">
            <CardContent className="flex flex-col items-center justify-center p-12 text-center">
              ...
              <CardDescription className="mb-4">
                Assign blocklists to specific API keys for granular control over which endpoints are protected.
              </CardDescription>
            </CardContent>
          </Card>
        </div>
              ...
              <div className="space-y-2">
                <div>
                  ...
                  <div className="mt-1 text-sm">
                    {blocklist.usage?.api_key_count > 0 ? (
                      <div className="space-y-1">
                        <p>
                          Used by <strong>{blocklist.usage.api_key_count}</strong> API key{blocklist.usage.api_key_count !== 1 ? 's' : ''} 
                          {' '}across <strong>{blocklist.usage.project_count}</strong> project{blocklist.usage.project_count !== 1 ? 's' : ''}:
                        </p>
                        <ul className="list-disc list-inside ml-2 text-muted-foreground">
                          {blocklist.usage.project_names?.map((projectName: string, idx: number) => (
              ...
              {deleteAssociatedKeys.length > 0 ? (
                <div className="space-y-4 mt-4">
                  <p className="text-destructive font-medium">
                    Cannot delete: {deleteAssociatedKeys.length} API key(s) are using this IP blocklist.
                  </p>
                  ...
                  {deleteAssociatedKeys.length === 0 && deleteTarget && (
                    <div className="mt-4 pt-4 border-t">
                      <p className="text-sm mb-2">All associated API keys have been deleted. You can now delete this IP blocklist.</p>
                      <Button
                        variant="destructive"
